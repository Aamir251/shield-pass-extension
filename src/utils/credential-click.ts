...
export const addCredentialItemClickListener = (credentialListWrappers: HTMLDivElement[], inputNodes: NodeListOf<Element>) => {

  credentialListWrappers.forEach(credentialListWrapper => {
    ...
    credentialListItems.forEach((item, index) => {
      ...
      heading.addEventListener("click", async function () {
        ...
        const credentialOverviewWrapper = item.querySelector(".sp-cred-detail") as HTMLDivElement
      ...
      const credentialUsernameNode = item.querySelector(".sp-cred-username") as HTMLParagraphElement | null;
      const credentialEmailNode = item.querySelector(".sp-cred-email") as HTMLParagraphElement | null
      const passwordFillButton = item.querySelector(".sp-cred-password") as HTMLButtonElement

      if (credentialUsernameNode) {
        console.log("Clicked ", credentialUsernameNode)
        credentialUsernameNode.addEventListener("click", handleClick.bind(null, credentialUsernameNode.innerText, inputNodes[index]))
      }
      if (credentialEmailNode) {
        console.log("Clicked ", credentialEmailNode)
        credentialEmailNode.addEventListener("click", handleClick.bind(null, credentialEmailNode.innerText, inputNodes[index]))
      }
